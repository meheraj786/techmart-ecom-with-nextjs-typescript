'use client'
import Link from 'next/link';
import React, { useState } from 'react'
import Flex from '../Layout/Flex';
import { Eye, EyeOff } from 'lucide-react';
import Container from '../Layout/Container';
import sideImg from '../../../public/SideImage.png'
import Image from 'next/image';

const Login: React.FC = () => {
    const [showPassword, setShowPassword] = useState(false);
  const [formData, setFormData] = useState({
    emailOrPhone: '',
    password: ''
  });

  const handleInputChange = (e: React.ChangeEvent <HTMLInputElement>) => {
    setFormData({
      ...formData,
      [e.target.name]: e.target.value
    });
  };

  const handleCreateAccount = () => {
    console.log('Create account clicked:', formData);
  };

  const handleGoogleSignup = () => {
    console.log('Sign up with Google clicked');
  };
  return (
    <div className="py-[120px] font-body">
      <Container>
      <div className="w-full  bg-white  overflow-hidden">
        <div className="flex flex-col lg:flex-row">
          
          <div className="lg:w-1/2  flex items-center justify-center relative">
<Image src={sideImg}  fill alt="registration image" />
          </div>

          <div className="lg:w-1/2 p-8 lg:p-12 flex items-center justify-center">
            <div className="w-full max-w-sm">
              
              <div className="mb-8">
                <h1 className="text-2xl font-bold text-gray-900 mb-2">
                  Log in to Exclusive
                </h1>
                <p className="text-gray-600 text-sm">
                  Enter your details below
                </p>
              </div>
              <div className="space-y-4">
                
                <div>
                </div>
                <div>
                  <input
                    type="text"
                    name="emailOrPhone"
                    placeholder="Email or Phone Number"
                    value={formData.emailOrPhone}
                    onChange={handleInputChange}
                    className="w-full px-0 py-3 border-0 border-b border-gray-300 focus:border-gray-500 focus:outline-none text-gray-700 placeholder-gray-500 bg-transparent"
                  />
                </div>

                <div className="relative">
                  <input
                    type={showPassword ? "text" : "password"}
                    name="password"
                    placeholder="Password"
                    value={formData.password}
                    onChange={handleInputChange}
                    className="w-full px-0 py-3 border-0 border-b border-gray-300 focus:border-gray-500 focus:outline-none text-gray-700 placeholder-gray-500 bg-transparent pr-8"
                  />
                  <button
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-0 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  >
                    {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
              </div>
              <Flex className="justify-between mt-6 mb-4">
              <button
                onClick={handleCreateAccount}
                className="w-full mb-2 bg-primary hover:bg-primary/90 cursor-pointer text-white font-medium py-3 rounded-md transition duration-200 "
              >
                Login
              </button>
              <p className='w-1/2 text-primary cursor-pointer text-right'>Forgot Password?</p>

              </Flex>

              <button
                onClick={handleGoogleSignup}
                className="w-full border border-gray-300 hover:border-gray-400 text-gray-700 font-medium py-3 rounded-md transition duration-200 flex items-center justify-center space-x-2 mb-6"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="#4285f4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34a853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#fbbc05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#ea4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <span>Sign up with Google</span>
              </button>

              <div className="text-center">
                <p className="text-gray-600 text-sm">
                  Do not have any account?{' '}
                  <Link href="/registration" className="text-gray-900 underline hover:text-gray-700 font-medium">
                    Signup
                  </Link>
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>

      </Container>
    </div>
  );
}

export default Login